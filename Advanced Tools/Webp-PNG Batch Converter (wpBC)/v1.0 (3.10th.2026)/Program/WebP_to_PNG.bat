...
set "toolPath=%~dp0dwebp.exe"

set /a converted=0
set /a skipped=0

if exist "%toolPath%" goto :ToolOk
echo Error: dwebp.exe not found next to this script.
...
:HandleFolderFile
if /I "%~x1"==".webp" goto :FolderIsWebP
echo Skipping non-WebP file: %~f1
set /a skipped+=1
exit /b
...
:ProcessFile
echo %~dpn1.png
"%toolPath%" "%~1" -o "%~dpn1.png"
if errorlevel 1 "%toolPath%" "\\?\%~f1" -o "\\?\%~dpn1.png"
