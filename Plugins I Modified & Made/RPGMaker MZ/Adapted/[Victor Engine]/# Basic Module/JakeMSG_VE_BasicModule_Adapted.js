/*
 * ==============================================================================
 * ** JakeMSG - VE Basic Module Adapted (MZ)
 * ------------------------------------------------------------------------------
 *  JakeMSG_VE_BasicModule_Adapted.js
 * ==============================================================================
 */

var Imported = Imported || {};
Imported['JakeMSG_VE_BasicModule_Adapted'] = '1.23';
Imported['VE - Basic Module'] = Imported['VE - Basic Module'] || Imported['JakeMSG_VE_BasicModule_Adapted'];
...
(function() {
    ...
    DataManager.loadDatabase = function() {
        ...
        PluginManager.requiredPlugin.call(PluginManager, 'JakeMSG_VE_BasicModule_Adapted');
    };
...
(function() {
    ...
    VictorEngine.Parameters.BasicModule.mev = String(parameters["Magic Evasion Name"]).trim();
    ...
    VictorEngine.Parameters.BasicModule.fdr = String(parameters["Floor Damage Name"]).trim();
    VictorEngine.Parameters.BasicModule.exr = String(parameters["Experience Rate Name"]).trim();
    VictorEngine.Parameters.BasicModule.ext = VictorEngine.Parameters.BasicModule.exr;
    ...
    VictorEngine.BasicModule.checkPlugins = function(name, req, ver) {
        var msg = '';
        var basicName = 'JakeMSG_VE_BasicModule_Adapted';
        var legacyBasicName = 'VE - Basic Module';
        ...
        if (ver && req && (!Imported[req] || Number(Imported[req]) < Number(ver))) {
            ...
            throw msg;
        } else if (req && (Imported[basicName] || Imported[legacyBasicName]) &&
            !this.loadedPlugins[basicName] && !this.loadedPlugins[legacyBasicName]) {
            ...
            if (name === basicName) {
                this.loadedPlugins[legacyBasicName] = true;
            } else if (name === legacyBasicName) {
                this.loadedPlugins[basicName] = true;
            }
        }
    };
    ...
    VictorEngine.waitAnimation = function(animationId) {
        ...
        if (!animation) {
            return 0;
        } else if (animation.frames && animation.frames.length) {
            return animation.frames.length * 4 + 1;
        } else {
            var timings = (animation.soundTimings || []).concat(animation.flashTimings || []);
            var maxFrame = timings.reduce(function(max, timing) {
                var frame = Number(timing.frame) || 0;
                var duration = Number(timing.duration) || 0;
                return Math.max(max, frame + duration);
            }, 0);
            if (animation.effectName) {
                return Math.max(maxFrame + 1, 60);
            } else {
                return maxFrame + 1;
            }
        }
    ...
    VictorEngine.paramText = function(name) {
        switch (name.toLowerCase()) {
            ...
            case 'exr':
            case 'ext':
                return this.Parameters.BasicModule[name.toLowerCase()] || this.Parameters.BasicModule.exr;
            case 'hp':
            case 'atk':
            case 'def':
            case 'agi':
    ...
    BattleManager.refreshStatus = BattleManager.refreshStatus || function() {};
    ...
    Game_BattlerBase.prototype.turnAwayFromTargets = function(targets) {
        var position = BattleManager.averageTargetsPositions(targets);
        this.turnAwayFromPosition(position.x, position.y);
    };
    ...
    VictorEngine.BasicModule.spritesetBattleUpdate = Spriteset_Battle.prototype.update;
    Spriteset_Battle.prototype.update = function() {
        VictorEngine.BasicModule.spritesetBattleUpdate.call(this);
    ...
    Window_BattleLog.prototype.waitForBattleAnimation = function(animationId, speed) {
        var animation = $dataAnimations[animationId];
        if (animation && animation.effectName) {
    ...
    VictorEngine.BasicModule.updateWaitModeBattleLog = Window_BattleLog.prototype.updateWaitMode;
    Window_BattleLog.prototype.updateWaitMode = function() {
        if (this._waitMode === 'animation') {
            var waiting = this._spriteset && this._spriteset.isAnimationPlaying();
            if (!waiting) {
                this._waitMode = '';
            }
            return waiting;
        } else {
            return VictorEngine.BasicModule.updateWaitModeBattleLog.call(this);
        }
    };
    ...
    Window_BattleLog.prototype.stackWaitFoAnimation = function(index, animationId, speed) {
        var animation = $dataAnimations[animationId];
        if (animation && animation.effectName) {
            this.setStackWaitMode(index, 'animation');
        } else {
            var speed = Math.max(speed, 1) || 1;
    ...
    Window_BattleLog.prototype.updateStackWaitMode = function(index) {
        ...
        if (waitMode.contains('animation')) {
            var waiting = this._spriteset && this._spriteset.isAnimationPlaying();
            if (waiting) {
                return true;
            } else {
                this.removeWaitMode(index, 'animation');
            }
        }
