//=============================================================================
// Dynamic Branching - for RPG Maker MZ version 1.8.0
// JakeMSG_DynamicBranching.js
//=============================================================================
 /*:
 * @plugindesc <DynamicBranching> for RPG Maker MZ version 1.8.0.
 * @author JakeMSG
 *
 * @help
 * With this plugin, you're capable of giving the player the choice to "branch"
 * out, by pressing customizable keys on their keyboard, to go to specific "Jump to Label"s.
 * Thus, you're able to "branch dynamically" during event processing, instead
 * of being restricted to branch/"Jump to Label" jump only at predetermined positions in the
 * event editor.
 * 
 * NOTE: Unlike the MV version, this uses "Jump to Label" instruction, NOT the normal "Label"
 * instruction!
 * And yes, much like in MV, you ONLY needed "Label" instructions for the plugin to work
 * (no need to use "Jump to Label" as well), in here, you ONLY need "Jump to Label"
 * instructions
 * 
 * 
 * With the given "Jump to Label"s, you can enable for the game to wait for specific keys
 * pressed to jump to the specific "Jump to Label"s for each key.
 * 
 ...
 * 
 * ------------------------
 * Instructions: "Jump to Label" Tags
 * ------------------------
 * 
 *     This plugin exclusively uses "Jump to Label"s for its Tags.
 *     All "Jump to Label"s are case insensitive and you can use as little/many whitespaces
 ...
 * for the "Revert Jump" / "Disable&Revert Jump" "Jump to Label" tags to go back to)
 ...
 * or until the matching <Disable Jump X> "Jump to Label", you can press the "X" (ASCII-code) 
 * key from the key board to jump to the matching <Jump Target X> "Jump to Label".
 * - If the matching <Jump Target X> "Jump to Label" is missing, it will forcefully close 
 ...
 * key, after being enabled by a previous matching <Enable Jump X> "Jump to Label"
 ...
 * * This "Jump to Label" works much like normal "Jump to Label"s work for the "Jump to "Jump to Label""
 ...
 * another <Enable Jump X> "Jump to Label" is used)
 * 
 * 
 * 
 * ================================ Experimental
...
// ==== Extends the "Jump to Label" instruction to check for the specific "Jump to Label"s used by the Plugin
JakeMSG.DynamicBranching.___Game_Interpreter_command119___ = Game_Interpreter.prototype.command119;
Game_Interpreter.prototype.command119 = function (params) {
  if (params[0].match(/<\s*Enable\s*Jump\s*(.*)\s*>/im)) {
    var key = parseInt(RegExp.$1);
    $gameTemp._currentJumpKeys.push(key); // Inserts key in the Key array
    $gameTemp._duringDBJump = false;
  } else if (params[0].match(/<\s*Disable\s*Jump\s*(.*)\s*>/im)) {
    var key = parseInt(RegExp.$1);
    ...
    $gameTemp._duringDBJump = false;
  } else if (params[0].match(/<\s*Jump\s*Target\s*(.*)\s*>/im)) {
    $gameTemp._duringDBJump = false;
  } else if (params[0].match(/<\s*Revert\s*Jump\s*(.*)\s*offset\s*(.*)\s*>/im) && $gameTemp._canRevertJumps[parseInt(RegExp.$1)]) {
  ...
  } else if ((params[0].match(/<\s*Disable\s*[&,+\s]*\s*Revert\s*Jump\s*(.*)\s*offset\s*(.*)\s*>/im) || params[0].match(/<\s*Revert\s*[&,+\s]*\s*Disable\s*Jump\s*(.*)\s*offset\s*(.*)\s*>/im)) && $gameTemp._canRevertJumps[parseInt(RegExp.$1)]) {
    var offset = parseInt(RegExp.$2);
    $gameTemp._canRevertJumps[parseInt(RegExp.$1)] = false;
    $gameTemp._duringDBJump = false;
    this.jumpTo($gameTemp._jumpStartIndexes[parseInt(RegExp.$1)] + offset);
  } else if (params[0].match(/<\s*Revert\s*Jump\s*(.*)\s*>/im) && $gameTemp._canRevertJumps[parseInt(RegExp.$1)]) {
    $gameTemp._currentJumpKeys.push(parseInt(RegExp.$1)); // Inserts key back in the Key array
    $gameTemp._canRevertJumps[parseInt(RegExp.$1)] = false;
    $gameTemp._duringDBJump = false;
    this.jumpTo($gameTemp._jumpStartIndexes[parseInt(RegExp.$1)]);
  } else if ((params[0].match(/<\s*Disable\s*[&,+\s]*\s*Revert\s*Jump\s*(.*)\s*>/im) || params[0].match(/<\s*Revert\s*[&,+\s]*\s*Disable\s*Jump\s*(.*)\s*>/im)) && $gameTemp._canRevertJumps[parseInt(RegExp.$1)]) {
    $gameTemp._canRevertJumps[parseInt(RegExp.$1)] = false;
    $gameTemp._duringDBJump = false;
    this.jumpTo($gameTemp._jumpStartIndexes[parseInt(RegExp.$1)]);
  } 

  return JakeMSG.DynamicBranching.___Game_Interpreter_command119___.call(this,params);
};
...
Game_Interpreter.prototype.finishDBJump = function (jumpKey, a8, a9, a10) {
  // ==== index of the actual "Jump to Label" 
