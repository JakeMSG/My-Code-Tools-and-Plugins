...
//
// Modified by JakeMSG (specified below)
// Date: 4/14th/2025
//=============================================================================
 ...
 *
 * @param ==== JakeMSG additions ====
 * @default
 * 
 * @param DefaultTipMessage
 * @parent ==== JakeMSG additions ====
 * @desc The default message to show on the QTE tip
 * @default "Do the QTE!"
 * 
 * @param DefaultTipDuration
 * @parent ==== JakeMSG additions ====
 * @desc The default duration of the QTE tip, in miliseconds (duration in which the QTE, initially, does not move)
 * @default 2000
 * 
 * @param StartSoundName
 * @parent ==== JakeMSG additions ====
 * @desc The name of the SE to be played on start of QTEt (after the Tip Duration of QTEt). If 0, no sound will be played.
 * @default Decision2
 *
 * @param StartSoundParam
 * @parent ==== JakeMSG additions ====
 * @desc The parameters for playing the start sound [volume,pitch,pan]
 * @default [90,100,0]
 * 
 * @help
 * ===================================================================
 ...
 * like going in the menu and then back.
 ...
 * 
 * ================ JakeMSG Additions! =======================
 * ==== New function to call on QTEs with Tip messages (and initial duration in which the QTE is frozen, to give time to the player to read the Tip)
 * The Tipped QTEs => QTEt 
 *
 * Function is similar (instead of "$gameMap.QTE()", you use "$gameMap.QTEt()"), and in the new function, 2 new parameters are added right at the beginning (the rest of the parameters remain the same):
 *
 * "$gameMap.QTEt(TipMessage,TipDuration,...)" 
 *
 * TipMessage => String message to show under the QTE (as a "Tip" to the player about the current QTE)
 * TipDuration => Initial Duration in Which the QTE is "frozen" (to give time to the player to read the Tip)
 */
...
Window_QTE.prototype.initialize = function(duration) {
	var parameters = PluginManager.parameters('JakeMSG_QTEWindowModification');
	...
	this.width = width;
	this.height = height;
	var a = eval(PluginManager.parameters('JakeMSG_QTEWindowModification').GaugeBack);
	var b = eval(PluginManager.parameters('JakeMSG_QTEWindowModification').GaugeFilling);
...
// ======== New variables added
	this._tipped = false;
	this._tippedAlready = 0;
	this._tipMessage = eval(parameters.DefaultTipMessage);
	this._tipDuration = eval(parameters.DefaultTipDuration);
    this._tipReadyStart = "-- Ready... --";
...
Window_QTE.prototype.update = function() {
	if (!this._frozen) {
		...
		this.contents.clear();
		var showTime = PluginManager.parameters('JakeMSG_QTEWindowModification').ShowTime;
...
// ================ JakeMSG addition
// ======== Insertion of the delay needed for the Tipped QTEs (QTEt)
        // ==== Checks to see if it's a tipped QTE (QTEt)
        if (this._tipped)
		{
			// ==== This is needed for my Workaround for the delay, via "Date.now()" in miliseconds
			if (this._tippedAlready < 5){this._tippedAlready++;}
			if (this._tippedAlready == 5)
			{
				this._tippedAlready++;
				this._frozen = true;	
				var startDelayMoment = Date.now();
				// ==== For some reason, the actual delay used is 133% of the one specified
				// == Therefore, I calculate the delay to insert off of the one in the parameters
				this._tipDuration = this._tipDuration / 3 * 2; 
			    while(startDelayMoment + this._tipDuration >= Date.now()){}; 
			    this._tipReadyStart = "== Start! ==";this.playStartSound(); 
				this._frozen = false;
			}
		}
...
// ================ JakeMSG addition
// ======== playStartSound function (sound used to signal the Start of the QTEt, after the Tip Duration of the QTEt)
Window_QTE.prototype.playStartSound = function() {
	var filename = PluginManager.parameters('JakeMSG_QTEWindowModification').StartSoundName;
    var ar = eval(PluginManager.parameters('JakeMSG_QTEWindowModification').StartSoundParam);
	var sound = {
		name: filename, 
		volume: ar[0], 
		pitch: ar[1], 
...
Window_QTE.prototype.playInput = function() {
	var filename = PluginManager.parameters('JakeMSG_QTEWindowModification').SuccessSoundName;
    var ar = eval(PluginManager.parameters('JakeMSG_QTEWindowModification').SuccessSoundParam);
	var sound = {
		name: filename, 
		volume: ar[0], 
		pitch: ar[1], 
...
Window_QTE.prototype.playSuccessSound = function() {
	var filename = PluginManager.parameters('JakeMSG_QTEWindowModification').ResultSuccessSound
    var ar = eval(PluginManager.parameters('JakeMSG_QTEWindowModification').ResultSuccessSoundParam);
	var sound = {
		name: filename, 
		volume: ar[0], 
		pitch: ar[1], 
...
Window_QTE.prototype.playFailureSound = function() {
	var filename = PluginManager.parameters('JakeMSG_QTEWindowModification').ResultFailureSound
    var ar = eval(PluginManager.parameters('JakeMSG_QTEWindowModification').ResultFailureSoundParam);
	var sound = {
		name: filename, 
		volume: ar[0], 
		pitch: ar[1], 
...
Window_QTE.prototype.getIconIndex = function(string) {
	var iconar = eval(PluginManager.parameters('JakeMSG_QTEWindowModification').IconArray);
	switch (string) {
		case "ok":
			return iconar[4];
			break;
...
Window_QTE.prototype.drawTime = function(mode) {
	var align = PluginManager.parameters('JakeMSG_QTEWindowModification').TextAlign
	var x;
	var y;
	...
	// ================ JakeMSG addition
	// ======== Insertion of the "drawTip" function call
    if (this._tipped) {this.drawTip();}
}

// ======================================================================================== JakeMSG addition
// ================ Draw Tip (called by DrawTime)
Window_QTE.prototype.drawTip = function() {
	// ==== This prepares and draws the Tip of the given QTEt
	var x = 0;
	var y = this.height /2 - 40;
	var width = this.width - 40;
	this.drawText(this._tipMessage,x,y,width,"center");

	// ==== This prepares and draws the "Ready/Start" text right under the Tip of the QTEt
	var x = 0;
	var y = this.height /2 + 15;
	var width = this.width - 40;
	this.drawText(this._tipReadyStart,x,y,width,"center");
}
...
Window_QTE.prototype.playOcarina = function(endButton) {
	...
	var ocarina = eval(PluginManager.parameters("JakeMSG_QTEWindowModification").OcarinaSounds);
	var filename = "0";
	if (input === "up") {
...
Scene_Battle.prototype.createAllWindows = function() {
	...
	this.createJakeMSG_QTEWindowModification()
};

Scene_Battle.prototype.createJakeMSG_QTEWindowModification = function() {
	this._JakeMSG_QTEWindowModification = new Window_QTE;
	this._JakeMSG_QTEWindowModification.visible = false;
    this.addWindow(this._JakeMSG_QTEWindowModification);
}

var _Astfgl_newSBIAIWA = Scene_Battle.prototype.isAnyInputWindowActive
Scene_Battle.prototype.isAnyInputWindowActive = function() {
	return _Astfgl_newSBIAIWA.call(this) || this._JakeMSG_QTEWindowModification.active
};

//BattleManager modifications
var _Astfgl_newBMIB = BattleManager.isBusy
BattleManager.isBusy = function() {
	return _Astfgl_newBMIB.call(this) || (SceneManager._scene._JakeMSG_QTEWindowModification.getResult() === "start")
};
...
Scene_Map.prototype.createJakeMSG_QTEWindowModification = function() {
	this._JakeMSG_QTEWindowModification = new Window_QTE;
	this.addChild(this._JakeMSG_QTEWindowModification);
	//this._JakeMSG_QTEWindowModificationIndex = this.children.length - 1;
	this._JakeMSG_QTEWindowModification.visible = false;
}

var _Astfgl_newSMIB = Scene_Map.prototype.isBusy
Scene_Map.prototype.isBusy = function() {
	return _Astfgl_newSMIB.call(this) || this._JakeMSG_QTEWindowModification.getResult() === "start"
};
...
Game_Map.prototype.QTE = function(mode,duration,sequence,visible,x,y,opacity,width,height,wrongInput,showTime) {
	var qte = SceneManager._scene._JakeMSG_QTEWindowModification
	...
	qte._endDur = eval(PluginManager.parameters("JakeMSG_QTEWindowModification").EndingDuration);
	qte._ending = false;
	if (visible) {
		qte.show()
	};
...
// ======================================================================================== JakeMSG addition
// ================ QTE call with Tips (QTEt)
Game_Map.prototype.QTEt = function(tipMessage,tipDuration,mode,duration,sequence,visible,x,y,opacity,width,height,wrongInput,showTime) {
	var qte = SceneManager._scene._JakeMSG_QTEWindowModification;

	if (height !== undefined) {qte.height = height} 
	qte.height = qte.height + 100; // We add more Height here, to fit our Tip text
	// ==== Introduces the new variables to use (and their defaults)
    qte._tipped = true;
	if (tipMessage !== undefined) {qte._tipMessage = tipMessage};
	if (tipDuration !== undefined) {qte._tipDuration = tipDuration};
...
Game_Map.prototype.castSandSM = function(user,target,actionAr,actionId) { //This command Requires YEP Battle Engine Core
	...
	$gameMap.castSequence(user,target,actionAr);
	SceneManager._scene._JakeMSG_QTEWindowModification._result = seq
	var targetIndex
	if (target instanceof Game_Actor) {
		targetIndex = $gameParty.battleMembers().indexOf(target)
	} else if (target instanceof Game_Enemy) {
...
Game_Interpreter.prototype.updateWaitMode = function() {
	return _Astfgl_newGIUWM.call(this) || (SceneManager._scene._JakeMSG_QTEWindowModification.getResult() === "start") 
...
Window_BattleLog.prototype.displayQTEFailure = function(target) {
	if (eval(PluginManager.parameters("JakeMSG_QTEWindowModification").DisplayFailText)) {
		var failText = PluginManager.parameters("JakeMSG_QTEWindowModification").FailText
		this.push('addText', failText);
	}
}

Window_BattleLog.prototype.displayNoMatch = function(target) {
	if (eval(PluginManager.parameters("JakeMSG_QTEWindowModification").DisplayNoMatchText)) {
		var failText = PluginManager.parameters("JakeMSG_QTEWindowModification").NoMatchText
		this.push('addText', failText);
	}
}
