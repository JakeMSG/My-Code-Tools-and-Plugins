 ...
 * Visual Angle: (Input a Number or JavaScript Formula, degrees; This only works for Default (Bar) Timed Attack, the others simply get displaced, but not rotated)
 * 
...
Game_Map.prototype.NotesProcChangeTA = function(o,notesTA) {
	...
	if(notesTA.match(/Visual[ ]?Angle:\s*([^;]*|.*$)/im)) {o.visualAngle = eval(String(RegExp.$1));}
}
...
SRD.TimedAttack.organizeInfo = function(o) {
	...
	else {o.visualYOffset = 0;}	if(o.info.match(/Visual[ ]?Angle:\s*(.*)/im)) {o.visualAngle = eval(String(RegExp.$1));}
	else {o.visualAngle = 0;}};
...
// helper for rotating coordinate pairs based on the visualAngle
TimedAttackSystem.prototype._applyAngleToCoords = function() {
	if (!this._visualAngle) return;
	var rad = this._visualAngle * Math.PI / 180;
	var cx = Graphics.width / 2;
	var cy = Graphics.height / 2;
	if (Array.isArray(this._x)) {
		for (var i = 0; i < this._x.length; i++) {
			var dx = this._x[i] - cx;
			var dy = this._y[i] - cy;
			this._x[i] = cx + dx * Math.cos(rad) - dy * Math.sin(rad);
			this._y[i] = cy + dx * Math.sin(rad) + dy * Math.cos(rad);
		}
	} else {
		var dx = this._x - cx;
		var dy = this._y - cy;
		this._x = cx + dx * Math.cos(rad) - dy * Math.sin(rad);
		this._y = cy + dx * Math.sin(rad) + dy * Math.cos(rad);
	}
};
...
TimedAttackSystem.prototype.loadItem = function(item) {
	...
	this._visualAngle = item.visualAngle || 0;
	...
	// ==== rotate the window by angle (always safe)
	this._window.rotation = this._visualAngle * Math.PI/180;
...
TimedAttackSystem.prototype.playCircleGame = function() {
	...
	if (!isSideview){
		...
		// = apply angle rotation to coordinates
		this._applyAngleToCoords();
	}
