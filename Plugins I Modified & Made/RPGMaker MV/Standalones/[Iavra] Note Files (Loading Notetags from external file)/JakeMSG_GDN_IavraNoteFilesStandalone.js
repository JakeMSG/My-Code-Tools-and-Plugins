...
============ Change Log ============
1.4 - 7.14th.2026
 * GDN JSON files now tolerate extra keys (eg: "GDN_Fields" from other plugins).
 * INF notetags can be used in Text Box, Script Call, and Skill Damage Formula fields.
 * New GDN keys: "GDN_TextBoxes", "GDN_ScriptCalls", "GDN_DamageFormulas".
1.3 - 7.17th.2025
 * Fully integrated the "Iavra Note Files" functionality into the plugin, making it a standalone plugin.
====================================
 * v1.3
 ...
 * ============================================================================
 * 
 * Create one or more files and store their locations inside the "File Path" parameter, for both "Global File Path" (GDN) 
 * and "Dedicated (Iavra) File Path" (INF), separated by comma for each if each has multiple files.. 
 * 
 * The files for the Dedicated (Iavra) Note Files are formatted like a normal JSON file, but with the notetags as Keys and their content as Values. Example:
 * 
 *  {
 *      "key1" : "",
 *      "key2" : "",
 ...
 *  }
 * 
 * Those files are flexible in how you want to format them, as long as they follow the JSON format. You can have as many notetags (keys) as you want, 
 * and you can name them however you like (since you will reference them manually in the notetags of your data entries). The only requirement is that 
 * the content of each notetag (value) has to be a String or an Array (which will be converted to a String with linebreaks).
 * 
 * Keep in mind, if multiple "key" entries are called the same, only the last one will be used, since they will overwrite each other.
 * 
 * You can "Nest" notetags inside each other, as long as you don't create cyclic references (eg: key1 references key2, and key2 references key1), since that will cause your game to crash.
 * 
 * You can also use INF files notetags in GDN files. Keep in mind, GDN file notetags are pre-pended above INF file notetags.
 * 
 * 
 * 
 * 
 * The files for the Global Notetags, however, have a fixed format for their keys (since the keys are read for each data type), like this:
 *
 *  {
 *       "GDN_Actors" : "",
 *       "GDN_Classes" : "", 
 ...
 *  }
 * 
 * Don't change the name of the built-in Keys above! (only change their values, the "" of each of them)
 * 
 * You may also include additional keys in the same file for other plugins (eg: "GDN_Fields").
 * Extra keys are loaded and stored normally; this plugin only prepends the keys listed above.
 * 
 ...
 * ============================================================================
 * 
 * ======== For Iavra Note Files (INF) functionality:
 * ==== Usage of the notetag to load the specific contents of the files:
 * <[notetag name] [key]>
 * 
 * == [notetag name] is the one you set in the plugin's parameters (default is "INFnote")
 * == [key] is the key of the notetag entry you want to load from your file (eg: "key1" or "key2" etc, from the examples above)
 * 
 * The same INF notetag syntax also works in Text Box lines, Script Call lines, and Skill Damage Formula fields.
 * See the "INF and GDN in editor instruction fields" section below.
 * 
 * 
 * 
 * ======== For Global Default Notetags (GDN) functionality:
 * ==== To manually disable the Global Default Notetags for a specific data entry:
 * <[Global Ignore Notetag]>
 * 
 * == [Global Ignore Notetag] is the one you set in the plugin's parameters (default is "GDNignore")
 * 
 * 
 * ======== For INF and GDN in editor instruction fields:
 * ==== Supported fields:
 * == Text Box lines (Show Text / 401 commands) on Map Events and Common Events
 * == Script Call lines (355 / 655 commands) and Conditional Branch script conditions (111, type 12) on Map Events and Common Events
 * == Skill Damage Formula fields in the database
 * 
 * ==== INF usage in those fields is the same as in notetags:
 * <[notetag name] [key]>
 * 
 * ==== GDN usage in those fields prepends the matching Global Default entry:
 * == "GDN_TextBoxes" for Text Box lines
 * == "GDN_ScriptCalls" for Script Call lines and Conditional Branch script conditions
 * == "GDN_DamageFormulas" for Skill Damage Formulas
 * 
 * ==== To skip GDN prepending for a specific instruction field, include the Global Ignore Notetag
 * in that field's text (INF replacement still runs afterward).
 * 
 * Processing order for instruction fields matches database notes: GDN prepend first, then INF replace.
 * 
 * 
 */
...
JakeMSG.Alterations_GDN.version = 1.4;
...
(function() {
    ...
    /**
     * Normalizes a JSON value into a loadable string.
     */
    ...
    /**
     * Merges parsed JSON objects into a target data store. Unknown keys are kept so other plugins can use them.
     */
    var mergeJsonData = function(target, result) {
        for (var key in result) {
            if (!Object.prototype.hasOwnProperty.call(result, key)) { continue; }
            target[key] = normalizeJsonValue(result[key]);
        }
    };

    /**
     * Runs a callback after GDN files load, then after INF files load.
     */
    var addProcessListener = function(callback) {
        GDN_addLoadListener(function() {
            INF_addLoadListener(callback);
        });
    };
    ...
    var GDNloadFile = function(file) {
        ...
        xhr.onload = function() {
            _GDNfiles[file] = true;
            mergeJsonData(_GDNdata, JSON.parse(xhr.responseText));
            if(isGDNReady()) { 
    ...
    var INFloadFile = function(file) {
        ...
        xhr.onload = function() {
            _INFfiles[file] = true;
            mergeJsonData(_INFdata, JSON.parse(xhr.responseText));
            if(isINFReady()) { 
    ...
    var GDN_DATA_KEYS = {
        ...
        "Enemies": "GDN_Enemies",
        "States": "GDN_States",
        "Tilesets": "GDN_Tilesets",
        "Map": "GDN_Map"
    };

    var GDN_INSTRUCTION_KEYS = {
        "TextBoxes": "GDN_TextBoxes",
        "ScriptCalls": "GDN_ScriptCalls",
        "DamageFormulas": "GDN_DamageFormulas"
    };

    var prependGDN = function(text, gdnKey) {
        text = text || '';
        _globalIgnore.lastIndex = 0;
        if (_globalIgnore.test(text)) { return text; }
        var prepend = _GDNdata[gdnKey] || '';
        return prepend ? prepend + text : text;
    };

    var processInstructionField = function(text, instructionType) {
        var gdnKey = GDN_INSTRUCTION_KEYS[instructionType];
        return replace(prependGDN(text, gdnKey));
    };

    var processEventCommandList = function(list) {
        if (!list) { return; }
        for (var i = 0; i < list.length; i++) {
            var command = list[i];
            if (!command || !command.parameters) { continue; }
            switch (command.code) {
            case 401:
                command.parameters[0] = processInstructionField(command.parameters[0], 'TextBoxes');
                break;
            case 355:
            case 655:
                command.parameters[0] = processInstructionField(command.parameters[0], 'ScriptCalls');
                break;
            case 111:
                if (command.parameters[0] === 12 && command.parameters[1] !== undefined) {
                    command.parameters[1] = processInstructionField(command.parameters[1], 'ScriptCalls');
                }
                break;
            }
        }
    ...
    var processEventPages = function(eventData) {
        if (!eventData || !eventData.pages) { return; }
        for (var i = 0; i < eventData.pages.length; i++) {
            var page = eventData.pages[i];
            if (page) { processEventCommandList(page.list); }
        }
    };

    var processSkillDamageFormulas = function(skills) {
        if (!skills) { return; }
        for (var i = 0; i < skills.length; i++) {
            var skill = skills[i];
            if (skill && skill.damage && skill.damage.formula !== undefined) {
                skill.damage.formula = processInstructionField(skill.damage.formula, 'DamageFormulas');
            }
        }
    };
    ...
    var GDNprepend = function(note, dataType) {
        var gdnKey = GDN_DATA_KEYS[dataType];
        return gdnKey ? prependGDN(note, gdnKey) : note;
    };
...
DataManager.onLoad = function(object) {
    var array;
    var objectTypeGDN = "_";
    var mapEventsToProcess = [];
    var commonEventsToProcess = [];
    ...
    if (Array.isArray(array)) {
        ...
        for (var i = 0; i < array.length; i++) {
            ...
            if (objectTypeGDN == "MapEvents" && data && data.pages) {
                mapEventsToProcess.push(data);
            }
            if (object === $dataCommonEvents && data && data.list) {
                commonEventsToProcess.push(data);
            }
        }
    }
    if (mapEventsToProcess.length > 0) {
        addProcessListener(function() {
            for (var j = 0; j < mapEventsToProcess.length; j++) {
                processEventPages(mapEventsToProcess[j]);
            }
        });
    }
    if (commonEventsToProcess.length > 0) {
        addProcessListener(function() {
            for (var j = 0; j < commonEventsToProcess.length; j++) {
                processEventCommandList(commonEventsToProcess[j].list);
            }
        });
    }
    if (object === $dataSkills && Array.isArray(array)) {
        addProcessListener(function() {
            processSkillDamageFormulas(array);
        });
    }
